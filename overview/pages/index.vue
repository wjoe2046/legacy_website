<template>
  ...
  <v-container class="overviewContainer">
    <v-row>
      ...
      <p><span id="databasePrivacy" class="subTitleLine">GPS Database Privacy Model</span></p>
      ...
      <p>The first server, Server A, will handle anonymisation using the methods described in the <span style="font-style:italic">Heatmap Privacy Model</span> section above. Users will send their GPS data to Server A first without any other information, and Server A will return the now-anonymised data to them.</p>
      <p>They will then upload the anonymised data, along with their infection status, to the second server, Server B. Server B will take this data and add it to our epidemiological simulation, generating a heatmap.</p>
    ...
    <p>&nbsp;</p>
      ...
      <p>The track to development requires (1) more volunteers to help us with skill sets listed on our <a href="https://www.covid19risk.com/collaborate.html">collaborate page</a> and (2) funding for, at a minimum, cloud services. </p>
      ...
      <p>To have a significant impact the technology will require a significant adoption rate. To detect contact events using bluetooth both individuals must have the app installed at the time of transmission. Assuming ‘P’ percent of the population uses the app, ‘A’  percent of transmission events between app users are detected, and ‘T’ percent of infected are tested, then T*A*P^2 percent of total transmissions are detected (if app users are homogeneous in the population). The figure below examines the percentage of detectable transmission events in countries with different testing rates as a function of app usage. The analysis assumes that these events are uncorrelated, which may significantly under-estimate detected transmissions due to missing chain-reaction testing. Ongoing modelling work is being done to investigate the dynamics of rapid tracing and testing along infection chains.</p>

      <p style="font-size:30px;color:red;">need three graph imgs, see comment in google doc</p>  
      <p>Transmission Detection vs. App Usage curves for testing rates [0.05, 0.1, 0.2, 0.4, 0.8]. BLE_Acc gives the detection rate of transmission events between app users. Work derived from this <a href="https://colab.research.google.com/drive/1V7yuwuVHK-WXP5p_LWmJ77NuhNb2Lvid">Monte Carlo Model</a>.</p>
      <p>In order to investigate disease dynamics with clusters of app users, the model developed by Hellewel et al was modified to account for two sub-populations, one that uses the app and another that doesn’t. The modified implementation is available on Github and currently assumes 90% app tracing accuracy and 50% non-app tracing accuracy (assuming health systems are partially overwhelmed) <a href="https://github.com/covid19risk/impact-sim">Github Link</a> .
This model was used to estimate the average number of infections caused in 6 weeks due to a single imported case. Results show a significant reduction in risk for clusters of app users that are partially distanced from non app users. This is important because it provides an additional incentive for individuals to use the app even when the overall adoption rate is low. As the proportion of the population using the app increases the risk for the entire population is reduced and most outbreaks are contained. </p>

       <p> Expected infections for total population (top) and app population (bottom) adjusted for relative population size. </p>
